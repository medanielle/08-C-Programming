...
TEST(TestCase1, findVowelsTest_cornerCases)
{
	struct Vowels v = { 0,0,0,0,0,0 };
	char *sentence = "";
	ASSERT_EQ(ERROR_EMPTY, countVowels(sentence, &v));
	sentence = NULL;
	ASSERT_EQ(ERROR_EMPTY, countVowels(sentence, &v));
}


TEST(TestCase2, findVowelsTest_normalCases)
{

		struct Vowels v = { 0,0,0,0,0,0 };
		char *sentence = "Yesterday, all my trouble seemed so far away.";
		...
		char *sentence2 = "Why is it that C programming taxes your brain so much? You can spend hours looking at it and you end up with a long cry.";
